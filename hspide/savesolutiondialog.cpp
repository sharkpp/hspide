...
bool CSaveSolutionDialog::setSolution(CWorkSpaceItem* item)
{
	...
	bool noneSolution = CWorkSpaceItem::Solution != item->type();
	rootItem->addChild(newItem = new QTreeWidgetItem());
	newItem->setText(FileNameColumn, noneSolution ? tr("(untitled)") : item->text());
	newItem->setData(FileNameColumn, WorkSpaceItemRole, qVariantFromValue((void*)item) );
	newItem->setHidden(true);
	newItem->setFlags(newItem->flags()|Qt::ItemIsEditable);
	if( item->isUntitled() || noneSolution ) { // 無題の場合上書き保存を無効にする
		newItem->setData(SaveKindColumn, DisableIndexRole, qVariantFromValue<int>(OverwriteSave));
		newItem->setData(SaveKindColumn, Qt::DisplayRole,  qVariantFromValue<int>(noneSolution ? WithoutSave : SaveAs));
	} else {
